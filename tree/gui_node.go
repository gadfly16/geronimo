...
func (n *GUINode) run() {
	...
	Tree.Sys.TreeUpdater.Ask(SystemUser, M_Subscribe, n.Tag, n.Owner)
	// Start satelites
		...
		case wm := <-n.wsr:
			...
			case SubscribeWsMsgKind:
				if wm.NodeID != n.ID {
					...
					sn.Notify(n.Owner, M_Subscribe, n.Tag)
				}
			...
			case UnsubscribeWsMsgKind:
				if wm.NodeID != n.ID {
					...
					sn.Notify(n.Owner, M_Unsubscribe, n.Tag)
				}
				...
				for nid := range n.subNodes {
					if nid != n.Head.ID {
						go func() {
							...
							sn.Ask(n.Owner, M_Unsubscribe, n.Tag)
						}()
					}
				}
				// Unsubscribe from tree updater
				Tree.Sys.TreeUpdater.Ask(SystemUser, M_Unsubscribe, n.Tag, n.Owner)
